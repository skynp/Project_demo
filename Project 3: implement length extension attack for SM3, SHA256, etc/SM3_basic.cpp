...
* 向左循环移位
*/
#define LeftRotate(message, bits) ( (message) << (bits) | (message) >> (32 - (bits)) )
...
void SM3Init(SM3Context* context)
{
	context->intermediateHash[0] = 0x04266AA7;
	context->intermediateHash[1] = 0x6C12BF4C;
	context->intermediateHash[2] = 0x2415FF9E;
	context->intermediateHash[3] = 0x6B546484;
	context->intermediateHash[4] = 0xEB992E82;
	context->intermediateHash[5] = 0xF017DA2A;
	context->intermediateHash[6] = 0xA9D9FDA7;
	context->intermediateHash[7] = 0x7D26BFE5;
}
/*
* 处理消息块
*/
void SM3ProcessMessageBlock(SM3Context* context)
{
	...
	cout << "压缩消息前" << endl;
	for (int i = 0; i < 8; i++)
		printf(" %X", context->intermediateHash[i]);
	cout << endl;
	/* 消息压缩 */
	...
	unsigned int messageLen, unsigned char digest[SM3_HASH_SIZE])
{
	...
	if (remainder <= 55)
	{
		...
		* 故将高 4 个字节赋为 0 。*/
		...
		cout << context.messageBlock << endl;
		SM3ProcessMessageBlock(&context);
	}
	else
	{
	...
	if (IsLittleEndian())
		for (i = 0; i < 8; i++)
			ReverseMessage(context.intermediateHash + i);
